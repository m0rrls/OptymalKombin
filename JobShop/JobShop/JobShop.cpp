...
int main()
{
	...
	std::vector<std::pair<Timeline, Timeline>> afterCross, afterMut, nextPop;
	nextPop = populacja;
	for (int kek = 0; kek < repeats; kek++)
	{
		...
		nextPop = afterCross;
		nextPop.insert(nextPop.end(), populacja.begin(), populacja.end());
		populacja = wybierz(nextPop, sizeOfPopulation);
